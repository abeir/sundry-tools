...
void Transformations::preRender() {
    // 编译着色器
    const char* vShaderPath = Util::Res::ShaderFile("transformations.vs");
    const char* fShaderPath = Util::Res::ShaderFile("transformations.fs");
...
void Transformations::render() {
    ...
    transform = glm::rotate(transform, (float)glfwGetTime(), glm::vec3(0.0f, 0.0f, 1.0f));  // 沿z轴旋转

    shader->Use();
    // 设置矩阵
    int transformLocation = shader->GetUniformLocation("transform");
